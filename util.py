...
class DATA_LOADER(object):
    ...
    def read_aptos(self, opt):
        ...
        self.test_feature = np.load(os.path.join(opt.dataroot, 'resnet101', 'test_features.npy'))  # (171, 2048)
        self.test_label = np.load(os.path.join(opt.dataroot, 'resnet101', 'test_targets.npy'))
        ...
        self.seenclasses = [0, 1, 3, 4]
        self.unseenclasses = [2]
        self.attribute_seen = self.attribute[self.seenclasses, :]

        # 提取标签为 1 和 2 的测试数据
        indices_seen = (self.test_label == 0) | (self.test_label == 1) | (self.test_label == 3) | (self.test_label == 4)
        self.test_seen_feature = self.test_feature[indices_seen]
        self.test_seen_label = self.test_label[indices_seen]

        # 提取标签为 0 的测试数据
        indices_unseen = self.test_label == 2
